...
__global__ void matrixMultiply(float *A, float *B, float *C, int numARows,
  ...
  float Pvalue = 0;
  int innnerWidth = numBRows;
  for(int m = 0; m<(innnerWidth-1)/TILE_WIDTH+1;++m){
    if(Row < numARows && m*TILE_WIDTH+tx<innnerWidth){
      subTileM[ty][tx] = A[Row*numAColumns + (m*TILE_WIDTH+tx)];
    }else{
      subTileM[ty][tx] = 0;
    }
    if(m*TILE_WIDTH+ty < innnerWidth && Col<numBColumns){
...
int main(int argc, char **argv) {
  ...
  //Store the sizes of each matrix
  int c_size = numCRows * numCColumns * sizeof(float);
  int b_size = numBRows * numBColumns * sizeof(float);
  int a_size = numARows * numAColumns * sizeof(float);
